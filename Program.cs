...
builder.Services.AddAuthorization(options =>
{
    ...
    // Defines an authorization policy named "ManageRolesPolicy".
    // This policy requires the user to be authenticated and to have a "permissions" claim with the value "manage_roles".
    // Defines an authorization policy named "ManageRolesPolicy".
    // This policy requires the user to be authenticated and to have a "permissions" claim with the value "manage_roles".
    options.AddPolicy(AuthController.ManageRolesPolicy, policy =>
        policy.RequireAuthenticatedUser()
              .RequireClaim("permissions", "manage_roles")); // Assumes "permissions" claim type stores permission strings

    // Policy for blocking users
    // Defines an authorization policy named "BlockUsersPolicy".
    // Defines an authorization policy named "BlockUsersPolicy".
    options.AddPolicy(AuthController.BlockUsersPolicy, policy =>
        policy.RequireAuthenticatedUser()
              .RequireClaim("permissions", "block_users"));
