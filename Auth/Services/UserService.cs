    ...
    public class UserService : IUserService
    {
        ...
        public async Task<(bool Success, ProfileResponseDto? Profile, string Message)> GetUserProfileAsync(int userId)
        {
            try
            {
                ...
                // .Select() projects the User entity into a ProfileResponseDto, fetching only necessary fields.
                ...
                return (true, user, "Profile retrieved successfully.");
            }
            catch (Exception ex)
            {
        ...
        public async Task<(bool Success, string Message)> UpdateUserProfileAsync(int userId, UpdateProfileRequestDto profileRequest)
        {
            try
            {
                ...
                if (profileRequest.RealName != null && user.RealName != profileRequest.RealName)
                {
                    user.RealName = profileRequest.RealName;
                    changed = true;
                }
                if (profileRequest.Url != null && user.Url != profileRequest.Url)
                {
                    user.Url = profileRequest.Url;
                    changed = true;
                }
                if (profileRequest.Personal != null && user.Personal != profileRequest.Personal)
                {
                    user.Personal = profileRequest.Personal;
                    changed = true;
                }
